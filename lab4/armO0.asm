...
.L8:
    ...
    ; ldm/stm Rn!{regs} rn не включается в regs, без ! включается, с ! не читается из памяти
    ; push - частный случай обращения в мамять

    ; bl - частынй случай перехода
    ; при вызове bl происходит переход, адрес следующей команды сохраняется в lr

    ; с помощью pop мы сразц возвращаемся на ту команду которую сохранили, возврщаемся команду в pc
    
    ; bx - условный переход, x здесь - ...
    
    ; vmla s15 s13 s14 14 и 13 скалдываются и добавляются в 15
    ; vldmia r3!, {r13} считалось r13 записалось в r3
    ; s0 - s31 вещественные флоаты регитсры
    ; 
    ;
    ; оптимизация
    ; ну например начинает использовать 16 байтные векторные регитсры q
    ;
    ; strd r2, [r7, #40] Она выполняет операцию сохранения 64-битного значения из регистра
    ; Это ассемблерная инструкция, которая означает "загрузить" (load). 
    ; Она используется для чтения данных из памяти и помещения их в регистр. r3: Это целевой регистр, в который будет загружено значение из памяти
    ; Команда vmov s15, r3 используется в архитектуре ARM NEON (SIMD) и выполняет операцию перемещения значения из целочисленного регистра r3 в регистр с плавающей запятой s15.
    ; vcvt.f64.s32: Это инструкция, которая выполняет преобразование значения из 32-битного формата (s32) в значение с плавающей запятой двойной точности (f64).
    ; vstr.64 d0, [r7, #8]  выполняет операцию сохранения 64-битного значения из регистра с плавающей запятой d0 по указанному адресу, который вычисляется как смещение относительно значения, хранящегося в регистре r7.
    ; vmov.f64 d7, #1.0e+0 помещает значение 1.0 в регистр d7
    ;
    ;
    ;
    ;
